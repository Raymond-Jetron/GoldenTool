...
bool prepare_metadata()
{
	...
	parse_func_map[std::string("名称")] = [](char *field, GOLDEN_POINT *base, GOLDEN_SCAN_POINT *scan, GOLDEN_CALC_POINT *calc) {
		// 去掉引号和中括号
		memcpy(base->tag, field + 2, GOLDEN_MIN(GOLDEN_TAG_SIZE, strlen(field) - 4));
	};
...
bool create_point() 
{
	auto create_point_func = [&](const std::string &point_file_path) {
		...
		for (field_number = 0; field_number < fields_ptr.size(); ++field_number) {
			it_func = parse_func_map.find(fields_ptr[field_number]);
			if (it_func != parse_func_map.end()) {
				...
				cc.convert(fields_ptr[field_number], strlen(fields_ptr[field_number]), out_buf, 128);
				g_logger->error("Unrecognized fields {} : [{}].", field_number + 1, out_buf/*fields_ptr[field_number]*/);
			}
		}
		...
		while (getline(ifs, line))
		{
			...
			try {
				for (field_number = 0; field_number < line_parse_func.size(); ++field_number) {
					line_parse_func[field_number](fields_ptr[field_number], &base, &scan, &calc);
				}
			}
			catch (...) {
				g_logger->error("Parse failed, line number : {}, field number : {}", line_number, field_number + 1);
				return;
			}
